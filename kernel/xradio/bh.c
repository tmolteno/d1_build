...
static struct sk_buff *xradio_get_skb(struct xradio_common *hw_priv, size_t len)
{
	...
	if (len > SDIO_BLOCK_SIZE || !hw_priv->skb_cache) {
		...
		if (skb) {
			...
			if (skb) {
				dev_dbg(hw_priv->pdev, "get skb_reserved(%ld)!\n", alloc_len);
				skb_reserve(skb, WSM_TX_EXTRA_HEADROOM + 8 /* TKIP IV */
						    - WSM_RX_EXTRA_HEADROOM);
			} else {
				dev_dbg(hw_priv->pdev, "xr_alloc_skb failed(%ld)!\n", alloc_len);
			}
		}
	} else {
...
static int xradio_bh_rx(struct xradio_common *hw_priv, u16* nextlen) {
	...
	if (read_len < sizeof(struct wsm_hdr) || (read_len > EFFECTIVE_BUF_SIZE)) {
		dev_err(hw_priv->pdev, "Invalid read len: %zd", read_len);
		return -1;
	}
	...
	if (WARN_ON_ONCE(alloc_len > EFFECTIVE_BUF_SIZE)) {
		dev_err(hw_priv->pdev, "Read aligned len: %zd\n", alloc_len);
	}
	...
	*nextlen = READLEN(__le16_to_cpu(((__le16 *) data)[(alloc_len >> 1) - 1]));
	...
	if (WARN_ON(wsm_len > read_len)) {
		dev_err(hw_priv->pdev, "wsm is bigger than data read, read %zd but frame is %zd\n",
				read_len, wsm_len);
		ret = -1;
		goto out;
	}
	...
	if (txavailable) {
		...
		if (ret <= 0) {
			...
			if (tx_len > EFFECTIVE_BUF_SIZE) {
				dev_warn(hw_priv->pdev, "Write aligned len: %zd\n", tx_len);
			}
